...
def user_login_logic(body: LoginUserBody):
    ...
    # 将类加载数据到模型中
    user_model = UserDto.model_validate(user)
    # xx.model_dump() 返回模型的字段和值的字典
    # 返回表示 model_dump() 的 JSON 字符串，只有当转换为json，模型里面的编码规则(json_encoders)才生效
    user_data = user_model.model_dump_json()
